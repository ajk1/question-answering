...
public class OtherRanker extends AbstractRanker {
  ...
  public Double score(Question question, Passage passage) {
	...
	StringEditDistance compare = new StringEditDistance();
  	String[] qTokens = question.getSentence().toLowerCase().split("[^a-zA-Z]+");
	String[] pTokens = passage.getText().toLowerCase().split("[^a-zA-Z]+");
	
	int[] minDist = new int[qTokens.length];
	
	for (int q=0; q<qTokens.length; q++) {
		minDist[q] = qTokens[q].length();
		for (int p=0; p<pTokens.length; p++) {
			int distance = compare.LD(qTokens[q], pTokens[p]);
			if (distance < minDist[q]){
				minDist[q] = distance;
			}
		}
	}
	...
	int sumOfSquares = 0;
	for (int x = 0; x < minDist.length; x++){
		sumOfSquares += Math.pow(minDist[x], 2);
	}
	
	int length = 0;
	for (String s : qTokens)
		length += s.length();
	double score = 1 - ((double)sumOfSquares/length);
